class SelectProofOfAddressController < ApplicationController
  ...
  def no_documents
    ...
    redirect_based_on_evidence(selected_evidence_assuming_phones)
  end
  ...
  def select_proof
    ...
    if @form.valid?
      ...
      redirect_based_on_evidence(selected_evidence)
    else
...
private
  ...
  def redirect_based_on_evidence(evidence)
    idps_available = IDP_ELIGIBILITY_CHECKER_B.any?(evidence, current_identity_providers)
