...
# This is identical to ChooseACertifiedCompanyLoa2Controller except that it uses a different IDP recommendation engine
class ChooseACertifiedCompanyLoa2VariantController < ApplicationController
  include ChooseACertifiedCompanyAbout
  include ViewableIdpPartialController

  def index
    suggestions = IDP_RECOMMENDATION_ENGINE_NO_PERF_MANAGEMENT.get_suggested_idps(current_identity_providers_for_loa, selected_evidence, current_transaction_simple_id)
  ...
  def select_idp
    ...
    select_viewable_idp_for_loa(params.fetch('entity_id')) do |decorated_idp|
      session[:selected_idp_was_recommended] = IDP_RECOMMENDATION_ENGINE_NO_PERF_MANAGEMENT.recommended?(decorated_idp.identity_provider, current_identity_providers_for_loa, selected_evidence, current_transaction_simple_id)
