class ChooseACertifiedCompanyLoa2VariantController < ApplicationController
  ...
  def index
    grouped_identity_providers = IDP_RECOMMENDATION_GROUPER_VARIANT.group_by_recommendation(selected_evidence, current_identity_providers_for_loa, current_transaction_simple_id)
  ...
  def select_idp
    ...
    select_viewable_idp(params.fetch('entity_id')) do |decorated_idp|
      session[:selected_idp_was_recommended] = IDP_RECOMMENDATION_GROUPER_VARIANT.recommended?(decorated_idp.identity_provider, selected_evidence, current_identity_providers_for_loa, current_transaction_simple_id)
