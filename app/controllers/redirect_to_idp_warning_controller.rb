class RedirectToIdpWarningController < ApplicationController
  ...
  def continue_ajax
    ...
    if idp.viewable?
      select_registration(idp)
      outbound_saml_message = SESSION_PROXY.idp_authn_request(session['verify_session_id'], cookies[CookieNames::SECURE_COOKIE_NAME])
...
private
  ...
  def select_registration(idp)
    SESSION_PROXY.select_idp(session['verify_session_id'], cookies[CookieNames::SECURE_COOKIE_NAME], idp.entity_id, true)
