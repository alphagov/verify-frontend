class SelectProofOfAddressNoBankAccountController < ApplicationController
  def index
  ...
  def no_documents
    ...
    selected_answer_store.store_selected_answers('address_proof', 'debit_card' => false, 'credit_card' => false)
    redirect_based_on_evidence
  end

  def select_proof
    @form = SelectProofOfAddressNoBankAccountForm.new(params['select_proof_of_address_form'] || {})
    if @form.valid?
...
private
  ...
  def redirect_based_on_evidence
    idps_available = IDP_ELIGIBILITY_CHECKER_C.any?(selected_evidence_assuming_phones, current_identity_providers)
