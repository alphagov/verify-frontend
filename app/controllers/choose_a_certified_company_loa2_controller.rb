...
class ChooseACertifiedCompanyLoa2Controller < RedirectToIdpWarningController
  ...
  def select_idp
    if params[:entity_id].present?
      select_viewable_idp_for_registration(params.fetch("entity_id")) do |decorated_idp|
        session[:selected_idp_was_recommended] = recommendation_engine.recommended?(decorated_idp.identity_provider, current_available_identity_providers_for_sign_in, selected_evidence, current_transaction_simple_id)
