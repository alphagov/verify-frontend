class FurtherInformationController < ApplicationController
  def index
    @cycle_three_attribute = FURTHER_INFORMATION_SERVICE.get_attribute_for_session(session, cookies).new({})
    @transaction_name = current_transaction.name
  end

  def submit
    cycle_three_attribute_class = FURTHER_INFORMATION_SERVICE.get_attribute_for_session(session, cookies)
    @cycle_three_attribute = cycle_three_attribute_class.new(params['cycle_three_attribute'])
    if @cycle_three_attribute.valid?
      FURTHER_INFORMATION_SERVICE.submit(session, cookies, @cycle_three_attribute.sanitised_cycle_three_data)
  ...
  def cancel
    FURTHER_INFORMATION_SERVICE.cancel(session, cookies)
  ...
  def submit_null_attribute
    ...
    if cycle_three_attribute_class.allows_nullable?
      FURTHER_INFORMATION_SERVICE.submit(session, cookies, '')
