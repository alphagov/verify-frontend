class ChooseACertifiedCompanyVariantNoBankAccountController < ApplicationController
  def index
    if is_loa1?
      ...
      grouped_identity_providers = IDP_RECOMMENDATION_GROUPER_C.group_by_recommendation(selected_evidence, current_identity_providers, current_transaction_simple_id)
  ...
  def select_idp
    ...
    select_viewable_idp(params.fetch('entity_id')) do |decorated_idp|
      session[:selected_idp_was_recommended] = IDP_RECOMMENDATION_GROUPER_C.recommended?(decorated_idp.identity_provider, selected_evidence, current_identity_providers, current_transaction_simple_id)
  ...
  def about
    ...
    if @idp.viewable?
      @recommended = IDP_RECOMMENDATION_GROUPER_C.recommended?(@idp, selected_evidence, current_identity_providers, current_transaction_simple_id)
      render 'about'
    else
