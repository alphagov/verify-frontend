class FeedbackSourceMapper
  ...
  def is_feedback_source_valid(feedback_source)
    return true if feedback_source.to_s.starts_with?("CHOOSE_A_CERTIFIED_COMPANY_ABOUT_")

    @page_to_source_mappings.has_key?(feedback_source.to_sym)
  end

  def page_from_source(feedback_source, locale)
    route_name = route_name_from(feedback_source&.to_sym)
    if route_name.nil?
      nil
...
private
  ...
  def route_name_from(feedback_source)
    if feedback_source && feedback_source.to_s.starts_with?("CHOOSE_A_CERTIFIED_COMPANY_ABOUT_")
