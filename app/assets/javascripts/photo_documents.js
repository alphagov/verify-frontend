(function (global) {
    ...
    var photoDocuments = {
        init: function () {
            ...
            if (photoDocuments.$form.length === 1) {
                $.validator.addMethod('drivingLicenceDetailsValidation', function(value, element) {
                    return ($('#photo_documents_form_any_driving_licence_true').is(':checked') && (
                                $('#photo_documents_form_driving_licence').is(':checked') || $('#photo_documents_form_ni_driving_licence').is(':checked')
                            )
                            || ($('#photo_documents_form_any_driving_licence_false').is(':checked')))
        ...
        highlight: function(element, errorClass) {
            photoDocuments.$form.children('.form-group:first').addClass('error');
        },
        unhighlight: function(element, errorClass) {
            photoDocuments.$form.children('.form-group:first').removeClass('error');
        },
        ...
        setDrivingLicenceDetailsVisibility: function () {
            if (photoDocuments.hasAnyDrivingLicence()) {
                ...
                photoDocuments.$drivingLicenceDetails.addClass('js-hidden').removeClass('error')
                    .find('.selected').removeClass('selected').find('input').prop('checked', true);
            }
        },
