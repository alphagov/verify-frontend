  ...
  class Application < Rails::Application
    ...
    config.action_dispatch.default_headers = {
      ...
      'Content-Security-Policy' => "default-src 'self' ; " +
        ...
        # `echo "'sha256-"$(echo -n "document.cookie = 'seen_cookie_message=yes; expires=' + new Date(new Date(Date.now()).setMonth(new Date(Date.now()).getMonth() + 1)) + ';'" | openssl dgst -sha256 -binary | openssl enc -base64)"'"`
