...
RSpec.describe 'When the user visits the start page on IDP focused variant' do
  ...
  def given_im_on_the_start_variant_page(locale = 'en')
    set_cookies_and_ab_test_cookie!('idp_focused_v3' => 'idp_focused_v3_variant')
  ...
  context 'when there is no error' do
    ...
    it 'will display the start page in English' do
      visit '/start'
      expect(page).to have_content t('hub.start.ab_test_idp_focused_heading')
      expect(page).to have_css 'html[lang=en]'
      expect_feedback_source_to_be(page, 'START_PAGE_IDP_FOCUSED_VARIANT', '/start')
    end

    it 'will display the start page in Welsh' do
      visit '/dechrau'
      expect(page).to have_content t('hub.start.ab_test_idp_focused_heading', locale: :cy)
  ...
  context 'with JS disabled', js: false do
    it 'will display list of IDP logos' do
      ...
      expect(page).to have_css("input[src*='/small-idp-focused-variant/stub-idp-one.png']")
    end
    ...
    it 'will redirect the user on selection of the IDP' do
      ...
      given_im_on_the_start_variant_page
      expect_any_instance_of(StartIdpFocusedVariantController).to receive(:select_idp).and_call_original
  ...
  context 'with JS enabled', js: true do
    it 'will redirect the user on selection of the IDP' do
      ...
      given_api_requests_have_been_mocked!
      given_im_on_the_start_variant_page
      expect_any_instance_of(StartIdpFocusedVariantController).to receive(:select_idp_ajax).and_call_original
      when_i_select_an_idp
      then_im_at_the_idp
      and_the_language_hint_is_set
      and_the_hints_are_not_set
