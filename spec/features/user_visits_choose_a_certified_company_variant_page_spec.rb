...
describe 'When the user visits the choose a certified company page' do
  ...
  context 'user chose reluctant yes for mobile app installation' do
    it 'will display installation warning message' do
      page.set_rack_session(
        transaction_simple_id: 'test-rp',
        selected_answers: {
          documents: { driving_licence: true },
      ...
      expect(page).to have_content('You will have to download an app for IDCorp')
    end
  end
end
