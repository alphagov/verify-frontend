...
describe SelectDocumentsVariantController do
  before(:each) do
  ...
  context 'when form is valid' do
    ...
    it 'redirects to choose a certified company page when no further documents are required' do
    ...
    it 'redirects to choose a certified company page when no further documents are required (yes to both passport and DL)' do
      no_further_documents_evidence = { passport: 'true', any_driving_licence: 'true', driving_licence: 'great_britain' }.freeze
      post :select_documents, params: { locale: 'en', select_documents_form: no_further_documents_evidence }

      expect(subject).to redirect_to choose_a_certified_company_path
    end

    it 'redirects to choose a certified company page when no further documents are required (yes to DL, no to passport)' do
      no_further_documents_evidence = { passport: 'false', any_driving_licence: 'true', driving_licence: 'great_britain' }.freeze
      post :select_documents, params: { locale: 'en', select_documents_form: no_further_documents_evidence }
