...
RSpec.describe FurtherInformationService do
  ...
  it 'should fetch cycle 3 display data' do
    ...
    expect(session_proxy).to receive(:cycle_three_attribute_name).with('session_id').and_return(attribute_key)
  ...
  it 'should submit cycle 3 attribute value' do
    expect(session_proxy).to receive(:submit_cycle_three_value).with('session_id', 'value')
    service.submit('session_id', 'value')
  end

  it 'should submit cycle 3 cancel' do
    expect(session_proxy).to receive(:cycle_three_cancel).with('session_id')
