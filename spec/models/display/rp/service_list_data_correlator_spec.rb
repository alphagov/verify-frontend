...
module Display
  module Rp
    describe ServiceListDataCorrelator do
      ...
      let(:service_list_data_correlator) {
      ...
      before(:each) do
        ...
        allow(translator).to receive(:translate).with('rps.test-rp.taxon_name').and_return(public_taxon)
        allow(translator).to receive(:translate).with('rps.test-rp-2.taxon_name').and_return(public_taxon_2)
        allow(translator).to receive(:translate).with('rps.test-rp-3.taxon_name').and_return(public_taxon_3)
        allow(translator).to receive(:translate).with('rps.test-rp-4.taxon_name').and_return(public_taxon_4)
      end
      ...
      it 'should return transactions with an empty list when the transactions property is absent' do
        transaction_data = {}
        expect(service_list_data_correlator.correlate(transaction_data)).to eq([])
      end
    end
  end
end
