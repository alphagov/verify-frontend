...
module Display
  module Rp
    describe TransactionFilter do
      ...
      let(:rp_loa1) { Transaction.new("rp-loa1", "http://example-loa1.com", [LevelOfAssurance::LOA1, LevelOfAssurance::LOA2]) }
      let(:rp_loa2) { Transaction.new("rp-loa2", "http://example-loa2.com", [LevelOfAssurance::LOA2]) }
      let(:rp_name_only_loa2) { Transaction.new("rp-name-only-loa2", nil, [LevelOfAssurance::LOA2]) }
      ...
      it "should provide a list of LOA1 transactions from a list containing LOA1 and LOA2 transactions" do
        expect(subject.filter_by_loa(transactions, LevelOfAssurance::LOA1)).to eql([rp_loa1])
      end

      it "should provide a list of LOA2 transactions from a list containing LOA1 and LOA2 transactions" do
        expect(subject.filter_by_loa(transactions, LevelOfAssurance::LOA2)).to eql([rp_loa2, rp_name_only_loa2])
      end

      it "should return an empty list if it is given an empty list of transactions" do
        expect(subject.filter_by_loa(empty_transactions, LevelOfAssurance::LOA1)).to eql([])
      end
    end
  end
end
