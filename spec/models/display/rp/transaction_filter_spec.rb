...
module Display
  module Rp
    describe TransactionFilter do
      Transactions = Struct.new(:name_homepage)
      ...
      let(:transactions) { Transactions.new([rp_loa1, rp_loa2]) }
      let(:empty_transactions) { Transactions.new([]) }
      ...
      it "should provide a list of LOA2 transactions from a list containing LOA1 and LOA2 transactions" do
        expect(subject.filter_by_loa(transactions, "LEVEL_2")).to eql([rp_loa2])
      end
